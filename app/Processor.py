...
class Processor(File):
    ...
    def procesar_archivo(self, rutaArchivoEntrada, rutaArchivoSalida):
        error = ""
        data = self.read_txt(rutaArchivoEntrada)

        if len(data) == 4:
            self.validar_datos(data)

            resultado = self.generar_mensaje(data)
            self.crear_txt(resultado, rutaArchivoSalida)

        else:
            error ="La estructura del archivo no coincide con la definición de cuatro líneas"
    ...
    def validar_instrucciones(self, data):
        ...
        if len(insList) != 3:
            ...
            if error == "":
                ...
                elif instruccion1 != self.limpiar_mensaje(instruccion1):
                    error = "Existen caracteres repetidos en la instruccion 1 " + instruccion1
                elif instruccion2 != self.limpiar_mensaje(instruccion2):
                    error = "Existen caracteres repetidos en la instruccion 2 " + instruccion2
                else:
                    error = (
                        ""
    ...
    def generate_response(self, error):
        return {
            ...
            "message": error
            if (error != "")
            else "Archivo procesado correctamente, se genero el archivo con los resultados",
        }
